...
origins = [
    "frontend",
    "frontend:3000",
    "frontend.colasloth.com",
    "http://frontend.colasloth.com/",
    "http://frontend.colasloth.com",
    "127.0.0.1",
]
...
async def get_first_nth_items(
    ...
    result = {"items": fake_items_db[start : start + length]}
    if q:
        result.update({"hidden-query": q})
    else:
        result.update({"hidden-query": None})
    return result


@app.get("/api/v1/items_by_idx/{idx}")
async def get_items_by_idx(q: str = "ahmed", idx: int = Path(title="Index of an item")):
